...
FrogPilotLongitudinalPanel::FrogPilotLongitudinalPanel(FrogPilotSettingsWindow *parent) : FrogPilotListWidget(parent), parent(parent) {
  ...
  const std::vector<std::tuple<QString, QString, QString, QString>> longitudinalToggles {
    ...
    {"LowSpeedCruiseOverride", tr("Low Speed Cruise Override"), tr("<b>Allow the cruise control stalk to set speeds below the normal floor (~28 mph on Toyota).</b><br><br>When active, the set speed can go as low as ~11 mph. The HUD shows a cyan \"LOW\" badge when the override is in effect.<br><br><i>Only applies to cars with openpilot longitudinal control and without PCM cruise.</i>"), ""},
